...
D        IF i > pwdcount THEN GOTO DL
DV	     j = I
DVI      k = I
DX       IF CHECKPASS(username, words(i) + CHR(XLVII + j) + CHR(XLVII + k)) THEN GOTO DXXXV
DXV      k = k + I
DXVI     IF k < XI THEN GOTO DX
DXVII	 j = j + I
DXX      IF j < XI THEN GOTO DVI
DXXV	 i = i + I
DXXX	 GOTO D
DXXXV	 PRINT "found match!! for user " + username + CHR(X)
DXL      PRINT "password: " + words(i) + CHR(XLVII + j) + CHR(XLVII + k) + CHR(X)
DXLV	 END
DL       PRINT "no matches for user " + username + CHR(X)
DLV      END
